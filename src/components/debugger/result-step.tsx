...
export function ResultStep({
  ...
  return (
    <div className={cn("flex flex-col", className)}>
      ...
      {/* Print-only section: QR code + footer */}
      {schemeApplicationUrl && (
        <div className="hidden print-qr-section" style={{ display: "none" }}>
          <QRCodeDisplay url={schemeApplicationUrl} size={96} />
          <div>
            <p className="text-sm font-medium">
              {language === "kn" ? "ಅರ್ಜಿ ಸಲ್ಲಿಸಲು ಸ್ಕ್ಯಾನ್ ಮಾಡಿ" : "Scan to apply"}
            </p>
            <p className="text-xs text-gray-500 break-all">{schemeApplicationUrl}</p>
          </div>
        </div>
      )}
